...
public class PinyinTool {
	 ...
	 * @throws BadHanyuPinyinOutputFormatCombination
	 */
	public static String[] toPinYinArray(String text) throws BadHanyuPinyinOutputFormatCombination {
		return PinyinTool.toPinYin(text, " ", Type.LOWERCASE).split(" ");
	}
	 ...
	 * @return
	 */
	public static String getFirstStr(String... strArray) {
		List<String> list = new ArrayList<>();
		for (String str : strArray) {
			if (!StringUtils.isEmpty(str)) {
				list.add(str.length() > 0 ? str.substring(0, 1) : str);
			}
		}
		return StringSplit.join("", list);
	}
	 ...
	 * @return
	 */
	public static String toText(String... strArray) {
		List<String> list = new ArrayList<>();
		for (String str : strArray) {
			if (str != null) {
				list.add(str);
